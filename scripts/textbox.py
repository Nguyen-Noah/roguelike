...
class Textbox:
    def __init__(self, game, font, width, color=(255, 255, 255), return_event=None, show_cursor=True, autoclear=False):
        self.game = game
        self.buffer = TextInputBuffer(parent=self)
        try:
            self.font = self.game.text[font]
        except KeyError:
            self.font = None
        self.width = width
    ...
    @property
    def bound(self):
        return self.game.input.text_buffer == self.buffer
    
    def bind(self):
        self.game.input.set_text_buffer(self.buffer)

    def unbind(self):
        self.game.input.set_text_buffer()
    ...
    def surf(self):
        ...
        if (self.game.window.time % 1 < 0.6) and self.bound:
